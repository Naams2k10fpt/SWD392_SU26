...
string connectionString = Environment.GetEnvironmentVariable("LUCY_DB")
    ...
    ?? "Server=localhost;Database=lucy_phase2;User=root;Password=;AllowUserVariables=True;";
...
app.MapPost("/auth/register", async (RegisterRequest request) =>
{
    ...
    if (string.IsNullOrWhiteSpace(email) || string.IsNullOrWhiteSpace(displayName) || string.IsNullOrWhiteSpace(request.Password))
    {
