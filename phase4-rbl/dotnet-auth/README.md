...
API hiện ghi/đọc user trực tiếp từ MariaDB qua các bảng `users`, `roles`, `user_roles`. Mặc định dùng connection string `Server=localhost;Database=lucy_phase4;User=root;Password=;`; nếu máy có password hoặc database khác, set biến môi trường `LUCY_DB` trước khi chạy `dotnet run`.
