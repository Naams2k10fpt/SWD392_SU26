...
- The module reads/writes MariaDB tables `mentor_material_pins`, `learner_progress`, and `lms_transition_events`. Configure with `LUCY_JDBC_URL`, `LUCY_DB_USER`, and `LUCY_DB_PASSWORD` if your local database is not `jdbc:mysql://localhost:3306/lucy_phase3` with root/no password.
