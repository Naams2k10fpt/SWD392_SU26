...
Các module được triển khai đúng tinh thần RBL MVP/scaffold để học kiến trúc và
defense. Trước production cần Agora token thật, payment gateway thật,
idempotency/outbox, xác thực server-side cho room join và mọi Socket event,
logging/monitoring, CI stress environment, object storage cho audio/tài liệu và
kiểm chứng parser của file PDF English Stage 3.
...
Các yêu cầu chính trong action plan RBL đều đã có folder/code hoặc tài liệu chứng
minh theo từng phase. Các phần còn là scaffold gồm Agora token, payment gateway,
Socket.IO production cluster và object storage cho podcast/tài liệu. Điều này phù
hợp mục tiêu RBL/MVP: chứng minh kiến trúc, flow chức năng, khả năng defense và
