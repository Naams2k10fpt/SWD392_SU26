...
test("keeps web contracts and proxy boundaries explicit", async () => {
  ...
  assert.match(page, /Yêu cầu mật khẩu khi tham gia/);
  assert.doesNotMatch(page, /window\.prompt/);
  assert.match(page, /aria-labelledby="join-password-title"/);
  assert.match(page, /passwordError && <p className="error" role="alert">/);
  assert.match(page, /code === "ROOM_PASSWORD_REQUIRED"/);
  ...
  assert.match(realtimeServer, /scryptSync/);
  assert.match(realtimeServer, /ROOM_PASSWORD_REQUIRED/);
